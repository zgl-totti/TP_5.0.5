...
class Department extends Base{
    public function index(){
        ...
        $department= new \app\admin\model\Department();
    ...
    public function add(){
        if(request()->isAjax()){
            ...
            if($validate->check($data)){
                ...
                $info= \app\admin\model\Department::get($where);
                if($info){
                    $res['status'] = 5;
                    $res['info'] = '科室已存在';
                    return $res;
    ...
    public function edit(){
        if(request()->isAjax()){
            ...
            if($validate->check($data)){
                ...
                $data['addtime']=time();
                $department= \app\admin\model\Department::get($id);
                if($department->save($data)) {
                    $res['status'] = 1;
                    $res['info'] = '编辑成功';
                    return $res;
            ...
            $id=input('param.id');
            $info=\app\admin\model\Department::get($id);
