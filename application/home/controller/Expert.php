...
class Expert extends Base{
    public function index(){
        ...
        $list=\app\admin\model\Expert::where($where)->with('departments')->paginate(10,false,$param);
    ...
    public function add(){
        if(request()->isAjax()){
            ...
            if($validate->check($data)){
                ...
                $info= \app\admin\model\Expert::get($where);
                if($info){
                    $res['status'] = 5;
                    $res['info'] = '专家已存在';
                    return $res;
                ...
                $data['addtime']=time();
                $expert= new \app\admin\model\Expert();
                if($expert->save($data)) {
    ...
    public function edit(){
        if(request()->isAjax()){
            ...
            if($validate->check($data)){
                ...
                $data['addtime']=time();
                $expert= \app\admin\model\Expert::get($id);
                if($expert->save($data)) {
                    if(!$_FILES){
                        $res['status']=1;
                        $res['info']='添加成功！';
            ...
            $id=input('param.id');
            $info=\app\admin\model\Expert::get($id);
            $list=Department::all();
            $this->assign('list',$list);
