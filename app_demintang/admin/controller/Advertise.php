...
class Advertise extends Base{
    ...
    public function index(){
        ...
        $list=\app\common\model\Advertise::where($where)->paginate(10,false,$param);
    ...
    public function add(){
        if(request()->isPost()){
            ...
            if($validate->check($data)){
                $arr=\app\common\model\Advertise::get($data);
                if($arr){
                    $res['status']=5;
                    $res['info']='广告已存在！';
                    return $res;
                ...
                $data['addtime']=time();
                //把数据添加到数据库;
                $advertise= new \app\common\model\Advertise();
    ...
    public function operate(){
        if(request()->isAjax()){
            $id=input('post.id');
            $info=\app\common\model\Advertise::get($id);
            if($info){
    ...
    public function edit(){
        if(request()->isAjax()){
            ...
            if($validate->check($data)){
                ...
                $where['id']=['neq',$id];
                $arr=\app\common\model\Advertise::get($where);
                if($arr){
                    $res['status']=5;
                    $res['info']='广告已存在！';
                    return $res;
                ...
                $data['addtime']=time();
                $advertise=\app\common\model\Advertise::get($id);
                $row=$advertise->save($data);
                if($row){
                    //处理图片上传;
                    $file=request()->file('pic');
