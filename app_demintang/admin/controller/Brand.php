...
class Brand extends Base{
    public function index(){
        ...
        $list=\app\common\model\Brand::where($where)->paginate(10,false,$param);
    ...
    public function add(){
        if(request()->isAjax()){
            ...
            if($validate->check($data)){
                ...
                $info= \app\common\model\Brand::get($where);
                if($info){
                    $res['status'] = 5;
                    $res['info'] = '文章已存在';
                    return $res;
                ...
                $data['addtime']=time();
                $news= new \app\common\model\Brand();
                if($news->save($data)) {
                    $res['status'] = 1;
                    $res['info'] = '添加成功';
                    return $res;
    ...
    public function edit(){
        if(request()->isAjax()){
            ...
            if($validate->check($data)){
                ...
                $data['addtime']=time();
                $news= \app\common\model\Brand::get($id);
                if($news->save($data)) {
                    $res['status'] = 1;
                    $res['info'] = '编辑成功';
                    return $res;
            ...
            $id=input('param.id');
            $info=\app\common\model\Brand::get($id);
