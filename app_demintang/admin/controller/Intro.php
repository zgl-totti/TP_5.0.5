...
class Intro extends Base{
    public function index(){
        ...
        $list=\app\common\model\Intro::where($where)->paginate(10,false,$param);
    ...
    public function add(){
        if(request()->isPost()){
            ...
            if($validate->check($data)){
                ...
                $arr=\app\common\model\Intro::get($where);
                if($arr){
                    $res['status']=5;
                    $res['info']='已存在！';
                    return $res;
                ...
                $data['addtime']=time();
                //把数据添加到数据库;
                $intro= new \app\common\model\Intro();
                $row=$intro->save($data);
                if($row){
                    $res['status']=1;
                    $res['info']='添加成功！';
    ...
    public function operate(){
        if(request()->isAjax()){
            $id=input('post.id');
            $info=\app\common\model\Intro::get($id);
            if($info){
    ...
    public function edit(){
        if(request()->isAjax()){
            ...
            if($validate->check($data)){
                ...
                $data['addtime']=time();
                //把数据添加到数据库;
                $Intro=\app\common\model\Intro::get($id);
                $row=$Intro->save($data);
                if($row){
                    $res['status']=1;
                    $res['info']='编辑成功！';
